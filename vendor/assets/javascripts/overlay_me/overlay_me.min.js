...
// - keymaster - http://github.com/madrobby/keymaster - Copyright (c) 2011-2012 Thomas Fuchs
...
if(key==93||key==224)key=91;if(key in _mods){_mods[key]=false;for(k in _MODIFIERS)if(_MODIFIERS[k]==key)assignKey[k]=false;}};function resetModifiers(){for(k in _mods)_mods[k]=false;for(k in _MODIFIERS)assignKey[k]=false;}
...
key=key[0]
key=_MAP[key]||key.toUpperCase().charCodeAt(0);if(!(key in _handlers))_handlers[key]=[];_handlers[key].push({shortcut:keys[i],scope:scope,method:method,key:keys[i],mods:mods});}};function isPressed(keyCode){if(typeof(keyCode)=='string'){if(keyCode.length==1){keyCode=(keyCode.toUpperCase()).charCodeAt(0);}else{return false;}}
...
object.attachEvent('on'+event,function(){method(window.event)});};addEvent(document,'keydown',function(event){dispatch(event,_scope)});addEvent(document,'keyup',clearModifier);addEvent(window,'focus',resetModifiers);var previousKey=global.key;function noConflict(){var k=global.key;global.key=previousKey;return k;}
global.key=assignKey;global.key.setScope=setScope;global.key.getScope=getScope;global.key.deleteScope=deleteScope;global.key.filter=filter;global.key.isPressed=isPressed;global.key.getPressedKeyCodes=getPressedKeyCodes;global.key.noConflict=noConflict;if(typeof module!=='undefined')module.exports=key;})(this);(function(){var test=document.createElement('input');try{test.type='range';if(test.type=='range')
...
keys=_.without(keys,'parent_path');return shiftTofiles(tree[keys[0]]);};return displayTree=function(parent,tree){var dir,img,sub_dir,_i,_j,_len,_len1,_ref,_ref1,_results;_ref=Object.keys(tree);for(_i=0,_len=_ref.length;_i<_len;_i++){dir=_ref[_i];if(dir==='files'||dir==='parent_path'){continue;}
...
return _results;}};};return OverlaysPanel;})(OverlayMe.MenuItem);$o(function(){if(!OverlayMe.overlay_panel){return OverlayMe.overlay_panel=new OverlayMe.OverlaysPanel();}});}).call(this);(function(){OverlayMe.setLoaded();}).call(this);