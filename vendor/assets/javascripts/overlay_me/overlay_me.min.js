...
if(deep&&copy&&(OMjQuery.isPlainObject(copy)||(copyIsArray=OMjQuery.isArray(copy)))){if(copyIsArray){copyIsArray=false;clone=src&&OMjQuery.isArray(src)?src:[];}else{clone=src&&OMjQuery.isPlainObject(src)?src:{};}
...
if(value!==undefined){if(value===null){OMjQuery.removeAttr(elem,name);return;}else if(hooks&&"set"in hooks&&notxml&&(ret=hooks.set(elem,value,name))!==undefined){return ret;}else{elem.setAttribute(name,""+value);return value;}}else if(hooks&&"get"in hooks&&notxml&&(ret=hooks.get(elem,name))!==null){return ret;}else{ret=elem.getAttribute(name);return ret===null?undefined:ret;}},removeAttr:function(elem,value){var propName,attrNames,name,l,isBool,i=0;if(value&&elem.nodeType===1){attrNames=value.toLowerCase().split(rspace);l=attrNames.length;for(;i<l;i++){name=attrNames[i];if(name){propName=OMjQuery.propFix[name]||name;isBool=rboolean.test(name);if(!isBool){OMjQuery.attr(elem,name,"");}
...
this.image_src=image_src;this.image_id=OverlayMe.Overlays.urlToId(image_src);$o(this.el).attr('data-img-id',this.image_id);this.images_container=$o('#overlay_me_images_container');if(this.images_container.length<1){$o('body').append((new Backbone.View).make('div',{id:'overlay_me_images_container'}));this.images_container=$o('#overlay_me_images_container');}
this.default_css=$.extend({visibility:'hidden',opacity:0.5},options.default_css);if(!($o("#"+this.image_id,this.images_container).length>0)){$o(this.images_container).append(this.image());}
...
image_id=OverlayMe.Overlays.urlToId(src);if(!($o("#overlay_me_images_container #"+image_id).length>0)){_default_css=$.extend({visibility:'visible'},options.default_css);image=new OverlayMe.Overlays.Image(src,{destroyable:true,default_css:_default_css});OverlayMe.images_management_div.append(image.render());}
