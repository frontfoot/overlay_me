...
return;var scale;var isMac=navigator.platform=='MacIntel';var thumb={radius:isMac?9:6,width:isMac?22:12,height:isMac?16:20};var track='-moz-linear-gradient(top, transparent '+(isMac?'6px, #999 6px, #999 7px, #ccc 9px, #bbb 11px, #bbb 12px, transparent 12px':'9px, #999 9px, #bbb 10px, #fff 11px, transparent 11px')+', transparent)';var styles={'min-width':thumb.width+'px','min-height':thumb.height+'px','max-height':thumb.height+'px',padding:0,border:0,'border-radius':0,cursor:'default','text-indent':'-999999px'};var onChange=document.createEvent('HTMLEvents');onChange.initEvent('change',true,false);if(document.readyState=='loading')
document.addEventListener('DOMContentLoaded',initialize,true);else
initialize();function initialize(){Array.forEach(document.querySelectorAll('input[type=range]'),transform);document.addEventListener('DOMNodeInserted',onNodeInserted,true);}
function onNodeInserted(e){check(e.target);if(e.target.querySelectorAll)
Array.forEach(e.target.querySelectorAll('input'),check);}
function check(input,async){if(input.localName!='input'||input.type=='range');else if(input.getAttribute('type')=='range')
transform(input);else if(!async)
...
var getValue=function(){return''+value;};var setValue=function setValue(val){value=''+val;isValueSet=true;draw();delete slider.value;slider.value=value;slider.__defineGetter__('value',getValue);slider.__defineSetter__('value',setValue);};slider.__defineGetter__('value',getValue);slider.__defineSetter__('value',setValue);slider.__defineGetter__('type',function(){return'range';});['min','max','step'].forEach(function(prop){if(slider.hasAttribute(prop))
areAttrsSet=true;slider.__defineGetter__(prop,function(){return this.hasAttribute(prop)?this.getAttribute(prop):'';});slider.__defineSetter__(prop,function(val){val===null?this.removeAttribute(prop):this.setAttribute(prop,val);});});slider.readOnly=true;style(slider,styles);update();slider.addEventListener('DOMAttrModified',function(e){if(e.attrName=='value'&&!isValueSet){value=e.newValue;draw();}
else if(~['min','max','step'].indexOf(e.attrName)){update();areAttrsSet=true;}},true);slider.addEventListener('mousedown',onDragStart,true);slider.addEventListener('keydown',onKeyDown,true);slider.addEventListener('focus',onFocus,true);slider.addEventListener('blur',onBlur,true);function onDragStart(e){isClick=true;setTimeout(function(){isClick=false;},0);if(e.button||!range)
return;var width=parseFloat(getComputedStyle(this,0).width);var multiplier=(width-thumb.width)/range;if(!multiplier)
return;var dev=e.clientX-this.getBoundingClientRect().left-thumb.width/2-
(value-min)*multiplier;if(Math.abs(dev)>thumb.radius){isChanged=true;this.value-=-dev/multiplier;}
rawValue=value;prevX=e.clientX;this.addEventListener('mousemove',onDrag,true);this.addEventListener('mouseup',onDragEnd,true);}
...
return;rawValue+=(e.clientX-prevX)/multiplier;prevX=e.clientX;isChanged=true;this.value=rawValue;}
function onDragEnd(){this.removeEventListener('mousemove',onDrag,true);this.removeEventListener('mouseup',onDragEnd,true);}
function onKeyDown(e){if(e.keyCode>36&&e.keyCode<41){onFocus.call(this);isChanged=true;this.value=value+(e.keyCode==38||e.keyCode==39?step:-step);}}
...
max=min>100?min:100;step=isAttrNum(slider.step)&&slider.step>0?+slider.step:1;range=max-min;draw(true);}
...
<<<<<<< HEAD:overlay_me.min.js
element.style.setProperty(prop,styles[prop],'important');}})();(function(){window.OverlayMe={};OverlayMe.isLoaded=function(){return window.overlay_me_loaded;};OverlayMe.isMobile=function(){return navigator.userAgent.match(/(iPhone|iPod|iPad|Android)/);};OverlayMe.mustLoad=function(){return!OverlayMe.isLoaded()&&!OverlayMe.isMobile();};}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Draggable=(function(_super){__extends(Draggable,_super);Draggable.name='Draggable';function Draggable(){return Draggable.__super__.constructor.apply(this,arguments);}
Draggable.prototype.tagName='div';Draggable.prototype.initialize=function(attributes,options){var cssData;Draggable.__super__.initialize.call(this,attributes,options);if((cssData=localStorage.getItem(this.id))){return $(this.el).css(JSON.parse(cssData));}else{if(options.default_css!==void 0){return $(this.el).css(options.default_css);}}};Draggable.prototype.engageMove=function(event){var _this=this;event.preventDefault();this.moving=true;this.lastX=event.clientX;this.lastY=event.clientY;$(window).bind('mymousemove',function(event,mouseEvent){_this.updateOverlay(mouseEvent.clientX-_this.lastX,mouseEvent.clientY-_this.lastY);_this.lastX=mouseEvent.clientX;return _this.lastY=mouseEvent.clientY;});return $(this.el).addClass('on-move');};Draggable.prototype.endMove=function(event){this.moving=false;$(window).unbind('mymousemove');return $(this.el).removeClass('on-move');};Draggable.prototype.toggleMove=function(event){if(this.moving){return this.endMove(event);}else{return this.engageMove(event);}};Draggable.prototype.updateOverlay=function(x,y){var newX,newY;newX=parseInt($(this.el).css('left'))+x;newY=parseInt($(this.el).css('top'))+y;$(this.el).css({top:""+newY+"px",left:""+newX+"px",right:"auto"});return this.saveCss();};Draggable.prototype.saveCss=function(){var cssData;cssData={top:$(this.el).css('top'),left:$(this.el).css('left'),visibility:$(this.el).css('visibility'),opacity:$(this.el).css('opacity')};return localStorage.setItem(this.id,JSON.stringify(cssData));};Draggable.prototype.render=function(){return this.el;};return Draggable;})(Backbone.View);}).call(this);(function(){var drag_me_line,_this=this;if(OverlayMe.mustLoad()){OverlayMe.menu_box=new OverlayMe.Draggable({id:'overlay_me_dev_tools_menu'},{default_css:{left:document.documentElement.clientWidth-300+'px',top:'0px'}});drag_me_line=(new Backbone.View).make('div',{"class":'drag-me'},'Drag me');OverlayMe.Menu=(new Backbone.View).make('ul');$(OverlayMe.menu_box.el).append(drag_me_line);$(OverlayMe.menu_box.el).append(OverlayMe.Menu);$('body').append(OverlayMe.menu_box.render());$(drag_me_line).bind('mousedown',function(event){return OverlayMe.menu_box.toggleMove(event);});$(window).bind('mouseup',function(event){return OverlayMe.menu_box.endMove(event);});$(OverlayMe.menu_box).bind('toggle:visibility',function(event){var css;if($(OverlayMe.menu_box.el).css('visibility')==='visible'){css={visibility:'hidden'};}else{css={visibility:'visible'};}
$(OverlayMe.menu_box.el).css(css);return OverlayMe.menu_box.saveCss();});}}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.MenuItem=(function(_super){__extends(MenuItem,_super);MenuItem.name='MenuItem';function MenuItem(){return MenuItem.__super__.constructor.apply(this,arguments);}
MenuItem.prototype.tagName='li';MenuItem.prototype.className='menu-item';MenuItem.prototype.initialize=function(attributes,options){var _this=this;this.id=attributes.id;this.el.appendChild(this.collapseButton());this.title=this.make('label',{"class":'title'},attributes.title);$(this.title).bind('click',function(){return _this.toggleCollapse();});this.el.appendChild(this.title);this.content=this.make('div',{"class":'item-content'});this.el.appendChild(this.content);this.collapsed=(localStorage.getItem(""+this.id+"-collapsed")===''?false:true);return this.setCollapse(this.collapsed);};MenuItem.prototype.collapseButton=function(){var _this=this;this.collapseButton=this.make('a',{"class":'collaps-button'},'<span>o</span>');$(this.collapseButton).bind('click',function(){return _this.toggleCollapse();});return this.collapseButton;};MenuItem.prototype.toggleCollapse=function(){this.collapsed=!this.collapsed;this.setCollapse(this.collapsed);return this.saveState();};MenuItem.prototype.setCollapse=function(toCollapse){if(toCollapse){return $(this.el).addClass('collapsed');}else{return $(this.el).removeClass('collapsed');}};MenuItem.prototype.append=function(childElemt){return this.content.appendChild(childElemt);};MenuItem.prototype.render=function(){return this.el;};MenuItem.prototype.saveState=function(){return localStorage.setItem(""+this.id+"-collapsed",(this.collapsed?1:''));};return MenuItem;})(Backbone.View);}).call(this);(function(){var basics_panel,clear_all_button,hide_button,_this=this;if(OverlayMe.mustLoad()){basics_panel=new OverlayMe.MenuItem({id:"bacis-options",title:"Basics"});clear_all_button=(new Backbone.View).make('button',{onClick:"javascript: localStorage.clear(); window.location.reload()"},'Reset All');basics_panel.append(clear_all_button);hide_button=(new Backbone.View).make('button',{},'Hide menu (touch "h")');$(hide_button).bind('click',function(event){return $(OverlayMe.menu_box).trigger('toggle:visibility');});basics_panel.append(hide_button);$(OverlayMe.Menu).append(basics_panel.render());$(window).bind('keypress',function(event){if(event.charCode===104){return $(OverlayMe.menu_box).trigger('toggle:visibility');}});}}).call(this);(function(){OverlayMe.Overlays={};OverlayMe.Overlays.urlToId=function(url){return url.replace(/[.:\/]/g,'_').replace(/[^a-zA-Z0-9_\-]/g,'');};OverlayMe.unicorns=["http://fc07.deviantart.net/fs49/f/2009/200/b/3/Fat_Unicorn_and_the_Rainbow_by_la_ratta.jpg","http://www.deviantart.com/download/126388773/Unicorn_Pukes_Rainbow_by_Angel35W.jpg","http://macmcrae.com/wp-content/uploads/2010/02/unicorn.jpg","http://4.bp.blogspot.com/-uPLiez-m9vY/TacC_Bmsn3I/AAAAAAAAAyg/jusQIA8aAME/s1600/Behold_A_Rainbow_Unicorn_Ninja_by_Jess4921.jpg","http://www.everquestdragon.com/everquestdragon/main/image.axd?picture=2009%2F9%2FPaperPaperNewrainbow.png"];}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Overlays.DraggableImage=(function(_super){__extends(DraggableImage,_super);DraggableImage.name='DraggableImage';function DraggableImage(){return DraggableImage.__super__.constructor.apply(this,arguments);}
DraggableImage.prototype.initialize=function(attributes,options){var _this=this;DraggableImage.__super__.initialize.call(this,attributes,options);this.image=(new Backbone.View).make('img',{src:options.image_src});$(this.el).append(this.image);if($(this.el).css('left')==='auto'||$(this.el).css('left')===''){$(this.el).css('left','0px');}
=======
element.style.setProperty(prop,styles[prop],'important');}})();(function(){$('head').append('<link rel="stylesheet" href="/assets/overlay_me/style.css" type="text/css" />');$('head').append('<link rel="stylesheet" href="/stylesheets/overlay_me/style.css" type="text/css" />');}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Draggable=(function(_super){__extends(Draggable,_super);Draggable.name='Draggable';function Draggable(){return Draggable.__super__.constructor.apply(this,arguments);}
Draggable.prototype.tagName='div';Draggable.prototype.initialize=function(attributes,options){var cssData,_this=this;Draggable.__super__.initialize.call(this,attributes,options);if((cssData=localStorage.getItem(this.id))){$(this.el).css(JSON.parse(cssData));}else{if(options.default_css!==void 0){$(this.el).css(options.default_css);}}
return _.each(options.events_to_save_on,function(event_name){return $(window).bind(event_name,function(){return _this.saveCss();});});};Draggable.prototype.engageMove=function(event){var _this=this;event.preventDefault();this.moving=true;this.lastX=event.clientX;this.lastY=event.clientY;$(window).bind('mymousemove',function(event,mouseEvent){_this.updateOverlay(mouseEvent.clientX-_this.lastX,mouseEvent.clientY-_this.lastY);_this.lastX=mouseEvent.clientX;return _this.lastY=mouseEvent.clientY;});return $(this.el).addClass('on-move');};Draggable.prototype.endMove=function(event){this.moving=false;$(window).unbind('mymousemove');return $(this.el).removeClass('on-move');};Draggable.prototype.toggleMove=function(event){if(this.moving){return this.endMove(event);}else{return this.engageMove(event);}};Draggable.prototype.updateOverlay=function(x,y){var newX,newY;newX=parseInt($(this.el).css('left'))+x;newY=parseInt($(this.el).css('top'))+y;$(this.el).css({top:""+newY+"px",left:""+newX+"px",right:"auto"});return this.saveCss();};Draggable.prototype.saveCss=function(){var cssData;cssData={top:$(this.el).css('top'),left:$(this.el).css('left'),visibility:$(this.el).css('visibility'),opacity:$(this.el).css('opacity')};return localStorage.setItem(this.id,JSON.stringify(cssData));};Draggable.prototype.render=function(){return this.el;};return Draggable;})(Backbone.View);}).call(this);(function(){var drag_me_line,menu_box,_this=this;if(OverlayMe.mustLoad()){menu_box=new OverlayMe.Draggable({id:'dev-tools-menu'},{default_css:{left:document.documentElement.clientWidth-300+'px',top:'0px'},events_to_save_on:['save:menu']});drag_me_line=(new Backbone.View).make('div',{"class":'drag-me'},'Drag me');OverlayMe.Menu=(new Backbone.View).make('ul');$(menu_box.el).append(drag_me_line);$(menu_box.el).append(OverlayMe.Menu);$('body').append(menu_box.render());$(drag_me_line).bind('mousedown',function(event){return menu_box.toggleMove(event);});$(window).bind('mouseup',function(event){return menu_box.endMove(event);});}}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.MenuItem=(function(_super){__extends(MenuItem,_super);MenuItem.name='MenuItem';function MenuItem(){return MenuItem.__super__.constructor.apply(this,arguments);}
MenuItem.prototype.tagName='li';MenuItem.prototype.className='menu-item';MenuItem.prototype.initialize=function(attributes,options){var _this=this;this.id=attributes.id;this.el.appendChild(this.collapseButton());this.title=this.make('label',{"class":'title'},attributes.title);$(this.title).bind('click',function(){return _this.toggleCollapse();});this.el.appendChild(this.title);this.content=this.make('div',{"class":'item-content'});this.el.appendChild(this.content);this.collapsed=(localStorage.getItem(""+this.id+"-collapsed")===''?false:true);return this.setCollapse(this.collapsed);};MenuItem.prototype.collapseButton=function(){var _this=this;this.collapseButton=this.make('a',{"class":'collaps-button'},'<span>o</span>');$(this.collapseButton).bind('click',function(){return _this.toggleCollapse();});return this.collapseButton;};MenuItem.prototype.toggleCollapse=function(){this.collapsed=!this.collapsed;this.setCollapse(this.collapsed);return this.saveState();};MenuItem.prototype.setCollapse=function(toCollapse){if(toCollapse){return $(this.el).addClass('collapsed');}else{return $(this.el).removeClass('collapsed');}};MenuItem.prototype.append=function(childElemt){return this.content.appendChild(childElemt);};MenuItem.prototype.render=function(){return this.el;};MenuItem.prototype.saveState=function(){return localStorage.setItem(""+this.id+"-collapsed",(this.collapsed?1:''));};return MenuItem;})(Backbone.View);}).call(this);(function(){var basics_panel,clear_all_button,hide_button,_this=this;if(OverlayMe.mustLoad()){window.toggle_menu_visibility=function(){var css,menu;menu=$('#dev-tools-menu');if($(menu).css('visibility')==='visible'){css={visibility:'hidden'};}else{css={visibility:'visible'};}
$(menu).css(css);return $(window).trigger('save:menu');};basics_panel=new OverlayMe.MenuItem({id:"bacis-options",title:"Basics"});clear_all_button=(new Backbone.View).make('button',{onClick:"javascript: localStorage.clear(); window.location.reload()"},'Reset All');basics_panel.append(clear_all_button);hide_button=(new Backbone.View).make('button',{onClick:"javascript: toggle_menu_visibility()"},'Hide menu (touch "H")');basics_panel.append(hide_button);$(OverlayMe.Menu).append(basics_panel.render());$(window).bind('keypress',function(event){if(event.charCode===104){return toggle_menu_visibility();}});}}).call(this);(function(){OverlayMe.Overlays={};}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Overlays.DraggableImage=(function(_super){__extends(DraggableImage,_super);DraggableImage.name='DraggableImage';function DraggableImage(){return DraggableImage.__super__.constructor.apply(this,arguments);}
DraggableImage.prototype.initialize=function(attributes,options){var _this=this;DraggableImage.__super__.initialize.call(this,attributes,options);this.image=(new Backbone.View).make('img',{src:options.image_src});$(this.el).append(this.image);if(options.default_css){$(this.el).css(options.default_css);}
if($(this.el).css('left')==='auto'||$(this.el).css('left')===''){$(this.el).css('left','0px');}
>>>>>>> 34bbf72... attempt with an erb...:overlay_me/overlay_me.min.js
if($(this.el).css('top')==='auto'||$(this.el).css('top')===''){$(this.el).css('top','0px');}
$(this.el).bind('mousedown',function(event){return _this.toggleMove(event);});$(window).bind('mouseup',function(event){return _this.endMove(event);});$(this.el).bind('mouseover',function(event){return $(".overlay-image-block[data-img-id="+_this.id+"]").addClass('hovered');});return $(this.el).bind('mouseout',function(event){return $(".overlay-image-block[data-img-id="+_this.id+"]").removeClass('hovered');});};DraggableImage.prototype.fitDivToImage=function(){if(this.image.width>0){$(this.el).css('width',this.image.width);return $(this.el).css('height',this.image.height);}};DraggableImage.prototype.render=function(){return this.el;};return DraggableImage;})(OverlayMe.Draggable);}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Overlays.Image=(function(_super){__extends(Image,_super);Image.name='Image';function Image(){return Image.__super__.constructor.apply(this,arguments);}
...
this.image_src=image_src;this.image_id=OverlayMe.Overlays.urlToId(image_src);$(this.el).attr('data-img-id',this.image_id);this.images_container=$('#overlay_me_images_container');if(this.images_container.length<1){$('body').append((new Backbone.View).make('div',{id:'overlay_me_images_container'}));this.images_container=$('#overlay_me_images_container');}
this.default_css=$.extend({visibility:'hidden',opacity:0.5},options.default_css);if(!($("#"+this.image_id,this.images_container).length>0)){$(this.images_container).append(this.image());}
...
$(this.el).bind('click',function(event){return _this.flickCheckbox();});$(this.el).bind('mouseover',function(event){$(_this.image.el).css('opacity',1);$(_this.image.el).addClass('highlight');return $(_this.el).addClass('hovered');});return $(this.el).bind('mouseout',function(event){$(_this.image.el).removeClass('highlight');$(_this.el).removeClass('hovered');return $(_this.image.el).css('opacity',$(_this.slider)[0].value/100);});};Image.prototype.image=function(){this.image=new OverlayMe.Overlays.DraggableImage({id:this.image_id},{image_src:this.image_src,default_css:this.default_css});return this.image.render();};Image.prototype.checkbox=function(){var _this=this;this.checkbox=this.make('input',{type:"checkbox"});if($(this.image.el).css('visibility')==='visible'){this.checkbox.checked=true;}
$(this.checkbox).bind('click',function(e){e.stopPropagation();return _this.flickVisibility();});$(this.checkbox).bind('change',function(e){e.stopPropagation();return _this.flickVisibility();});return this.checkbox;};Image.prototype.delButton=function(){var _this=this;this.delButton=this.make('button',{"class":'del-button',title:'Delete'},'x');$(this.delButton).bind('click',function(e){return OverlayMe.dyn_manager.delImage(_this.image_id);});return this.delButton;};Image.prototype.flickCheckbox=function(){this.checkbox.checked=!this.checkbox.checked;return this.flickVisibility();};Image.prototype.flickVisibility=function(){this.image.fitDivToImage();if(this.checkbox.checked){$(this.image.el).css('visibility','visible');}else{$(this.image.el).css('visibility','hidden');}
...
image_id=OverlayMe.Overlays.urlToId(src);if(!($("#overlay_me_images_container #"+image_id).length>0)){_default_css=$.extend({visibility:'visible'},options.default_css);image=new OverlayMe.Overlays.Image(src,{destroyable:true,default_css:_default_css});OverlayMe.images_management_div.append(image.render());}
...
<<<<<<< HEAD:overlay_me.min.js
ContentDivManagementBlock.prototype.tagName='fieldset';ContentDivManagementBlock.prototype.className='content-mgnt-block';ContentDivManagementBlock.prototype.normal_zindex='0';ContentDivManagementBlock.prototype.over_zindex='5';ContentDivManagementBlock.prototype.initialize=function(){var contentCss,our_page_container_div,slider_block,unicorn_button,_this=this;our_page_container_div=this.make('div',{id:'overlay_me_page_container'});$('body').append(our_page_container_div);$('body > *').each(function(index,thing){if(!(thing.id.match(/^overlay_me/)||thing.tagName==='SCRIPT')){return $(our_page_container_div).append(thing);}});$("#overlay_me_page_container").css({'z-index':this.normal_zindex});if((contentCss=localStorage.getItem("#overlay_me_page_container"))){$("#overlay_me_page_container").css(JSON.parse(contentCss));}
unicorn_button=this.make('div',{"class":'unicorns',title:'Feeling corny?'});$(unicorn_button).bind('click',function(){return OverlayMe.dyn_manager.addImage(OverlayMe.unicorns[Math.floor(Math.random()*OverlayMe.unicorns.length)],{default_css:{opacity:1}});});$(this.el).append(unicorn_button);$(this.el).append(this.make('legend',{},'Page content'));slider_block=this.make('div',{"class":'slider-block'});$(this.el).append(slider_block);slider_block.appendChild(this.make('label',{},'Opacity'));slider_block.appendChild(this.contentSlider());$(this.el).append(this.zIndexSwitch());return this.bindEvents();};ContentDivManagementBlock.prototype.zIndexSwitch=function(){var block,label,_this=this;block=this.make('div',{"class":'zindex-switch'});this.zIndexSwitch=this.make('input',{type:"checkbox"});$(block).append(this.zIndexSwitch);setTimeout(function(){if($("#overlay_me_page_container").css('z-index')===_this.over_zindex){return _this.zIndexSwitch.checked=true;}},500);label=this.make('label',{},'Content on top (touch "c")');$(label).bind('click',function(){return $(_this.zIndexSwitch).trigger('click');});return $(block).append(label);};ContentDivManagementBlock.prototype.contentSlider=function(){return this.contentSlider=this.make('input',{id:"contentSlider",type:"range",value:$("#overlay_me_page_container").css('opacity')*100});};ContentDivManagementBlock.prototype.bindEvents=function(){var _this=this;$(this.contentSlider).bind('change',function(){$("#overlay_me_page_container").css('opacity',$(_this.contentSlider)[0].value/100);return _this.saveContentCss();});$(this.zIndexSwitch).bind('change',function(event){if(_this.zIndexSwitch.checked){$("#overlay_me_page_container").css({'z-index':_this.over_zindex});}else{$("#overlay_me_page_container").css({'z-index':_this.normal_zindex});}
return _this.saveContentCss();});return $(window).bind('keypress',function(event){if(event.charCode===99){return $(_this.zIndexSwitch).trigger('click');}});};ContentDivManagementBlock.prototype.render=function(){return this.el;};ContentDivManagementBlock.prototype.saveContentCss=function(){return localStorage.setItem("#overlay_me_page_container",JSON.stringify({opacity:$("#overlay_me_page_container").css('opacity'),'z-index':$("#overlay_me_page_container").css('z-index')}));};return ContentDivManagementBlock;})(Backbone.View);}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Overlays.ImagesManagementDiv=(function(_super){__extends(ImagesManagementDiv,_super);ImagesManagementDiv.name='ImagesManagementDiv';function ImagesManagementDiv(){return ImagesManagementDiv.__super__.constructor.apply(this,arguments);}
ImagesManagementDiv.prototype.tagName='fieldset';ImagesManagementDiv.prototype.id='images_mgnt';ImagesManagementDiv.prototype.initialize=function(){var check_all_label,hide_inactives_label,_this=this;$(this.el).append(this.make('legend',{},'Overlaying images'));this.controlBlock=this.make('div',{"class":'controls'});$(this.el).append(this.controlBlock);this.controlBlock.appendChild(this.checkAllbox());check_all_label=this.make('label',{},'All/None');$(check_all_label).bind('click',function(){return $(_this.checkAllBox).trigger('click');});this.controlBlock.appendChild(check_all_label);this.controlBlock.appendChild(this.hideInactivesBox());hide_inactives_label=this.make('label',{},'Hide Inactives');this.controlBlock.appendChild(hide_inactives_label);$(hide_inactives_label).bind('click',function(){return $(_this.hideInactivesBox).trigger('click');});this.overlaysListBlock=this.make('div',{"class":'overlays-list'});$(this.el).append(this.overlaysListBlock);return $(this.el).append(this.dynamicAddsBlock());};ImagesManagementDiv.prototype.append=function(block){return this.overlaysListBlock.appendChild(block);};ImagesManagementDiv.prototype.del=function(image_id){$(".overlay-image-block[data-img-id="+image_id+"]",this.el).remove();return $("#overlay_me_images_container #"+image_id).remove();};ImagesManagementDiv.prototype.dynamicAddsBlock=function(){var dynamicAddsBlock,push_image_button,_this=this;dynamicAddsBlock=this.make('div',{"class":'dynamic-adds'});dynamicAddsBlock.appendChild(this.make('label',{},'Add an image'));this.image_url_input=this.make('input',{type:'text',placeholder:"http://"});dynamicAddsBlock.appendChild(this.image_url_input);push_image_button=this.make('button',{},'Add');dynamicAddsBlock.appendChild(push_image_button);$(this.image_url_input).bind('keypress',function(e){if(e.keyCode===13){return _this.pushImage();}});$(push_image_button).bind('click',function(e){return _this.pushImage();});return dynamicAddsBlock;};ImagesManagementDiv.prototype.pushImage=function(){OverlayMe.dyn_manager.addImage(this.image_url_input.value);return this.image_url_input.value='';};ImagesManagementDiv.prototype.hideInactivesBox=function(){var _this=this;this.hideInactivesBox=this.make('input',{type:"checkbox","class":'hide-inactive'});if($(this.hideInactivesBox).css('visibility')==='visible'){this.hideInactivesBox.checked=true;}
...
return this.saveState();};ImagesManagementDiv.prototype.saveState=function(){};ImagesManagementDiv.prototype.render=function(){return this.el;};return ImagesManagementDiv;})(Backbone.View);}).call(this);(function(){var overlay_panel;if(OverlayMe.mustLoad()){overlay_panel=new OverlayMe.MenuItem({id:"overlay_panel",title:"Overlays"});overlay_panel.append(new OverlayMe.Overlays.ContentDivManagementBlock().render());OverlayMe.images_management_div=new OverlayMe.Overlays.ImagesManagementDiv();overlay_panel.append(OverlayMe.images_management_div.render());$(OverlayMe.Menu).append(overlay_panel.render());$(window).bind('mousemove',function(event){return $(window).trigger('mymousemove',event);});OverlayMe.dyn_manager=new OverlayMe.Overlays.DynamicManager();OverlayMe.dyn_manager.loadAll();OverlayMe.loadDefaultImage=function(){if(OverlayMe.dyn_manager.isEmpty()){return OverlayMe.dyn_manager.addImage('https://a248.e.akamai.net/assets.github.com/images/modules/about_page/octocat.png');}};$.ajax({url:'/overlay_images',dataType:'json',success:function(data){if(data.length===0){return OverlayMe.loadDefaultImage();}else{return $.each(data,function(index,img_path){return OverlayMe.images_management_div.append(new OverlayMe.Overlays.Image(img_path).render());});}},error:function(){return OverlayMe.loadDefaultImage();}});}}).call(this);(function(){style=$('<style type="text/css" rel="stylesheet"></style>');style.html('button{font-size:10px;margin:0 5px;padding:0 5px}#overlay_me_page_container{position:relative}#overlay_me_dev_tools_menu{position:absolute;z-index:990}#overlay_me_dev_tools_menu .drag-me{line-height:100%;display:block;color:black;font-size:.7em;text-align:center;background-image:-webkit-gradient(linear,0deg,0deg,color-stop(0%,#999),color-stop(30%,#ddd),color-stop(70%,#ddd),color-stop(100%,#999));background-image:-webkit-linear-gradient(0deg,#999,#ddd 30%,#ddd 70%,#999 100%);background-image:-moz-linear-gradient(0deg,#999,#ddd 30%,#ddd 70%,#999 100%);background-image:-o-linear-gradient(0deg,#999,#ddd 30%,#ddd 70%,#999 100%);background-image:-ms-linear-gradient(0deg,#999,#ddd 30%,#ddd 70%,#999 100%);background-image:linear-gradient(0deg,#999,#ddd 30%,#ddd 70%,#999 100%);padding:1px}#overlay_me_dev_tools_menu .drag-me:hover{cursor:move}#overlay_me_dev_tools_menu ul{list-style:none;margin:0;padding:0;border:0;font-size:100%;font:inherit;vertical-align:baseline}.menu-item{text-align:left;background-color:#CCC;border:1px solid rgba(255,255,255,0.2);width:250px}.menu-item a.collaps-button{cursor:pointer;position:absolute;padding-top:9px;padding-left:5px;width:13px;height:9px;background:url(data:image/png;base64,) no-repeat center center}.menu-item a.collaps-button span{display:none;color:yellow}.menu-item a.collaps-button span:hover{color:yellow}.menu-item label.title{padding-left:20px;color:white;cursor:pointer;width:237px;line-height:1.1em;font-size:14px;background:none}.menu-item.collapsed .item-content{display:none}.menu-item.collapsed a.collaps-button{background:url(data:image/png;base64,) no-repeat center center}#overlay_panel .unicorns{background:url(data:image/png;base64,) no-repeat center center;width:15px;height:15px;position:absolute;right:4px;top:56px}#overlay_panel label{margin:0 5px 0 0}#overlay_panel .content-mgnt-block,#overlay_panel #images_mgnt{text-align:left;padding:3px 4px;margin:2px;border:1px #777 solid;-webkit-border-radius:5px;-moz-border-radius:5px;-ms-border-radius:5px;-o-border-radius:5px;border-radius:5px}#overlay_panel .content-mgnt-block legend,#overlay_panel #images_mgnt legend{font-size:70%;padding:0 3px;margin-left:10px}#overlay_panel .slider-block{display:block}#overlay_panel .slider-block label{font-size:70%;vertical-align:top}#overlay_panel .slider-block input[type=range]{margin:0}#overlay_panel #images_mgnt{width:236px}#overlay_panel #images_mgnt .controls{padding-bottom:5px}#overlay_panel #images_mgnt .overlay-image-block{text-align:left;position:relative;width:236px;border:1px solid rgba(255,255,255,0.2);border-right:none;border-left:none}#overlay_panel #images_mgnt .overlay-image-block.hovered{background-color:rgba(255,255,0,0.5)}#overlay_panel #images_mgnt .overlay-image-block .del-button{position:absolute;right:0;top:0;margin:3px 7px;cursor:pointer;border:1px #AAA solid;font-size:80%;background-color:#444;color:white;font-weight:bold;padding:0 4px 2px 4px}#overlay_panel #images_mgnt .dynamic-adds label{font-size:75%}#overlay_panel #images_mgnt .dynamic-adds input{width:105px;font-size:10px}#overlay_panel input[type=checkbox],#overlay_panel label,#overlay_panel #contentSlider,#overlay_panel .zindex-switch{display:inline}#overlay_panel input[type=checkbox]{vertical-align:middle;margin:-3px 5px 0 0}#overlay_me_images_container{position:absolute;z-index:4;top:0;left:0}#overlay_me_images_container div{position:absolute;visibility:hidden}#overlay_me_images_container div.highlight{background:rgba(255,255,0,0.5);opacity:1}#overlay_me_images_container div.highlight img{opacity:.7}#overlay_me_images_container div:hover{cursor:move}#overlay_me_images_container img{position:absolute;top:0;left:0}');$("head").append(style);window.overlay_me_loaded=true;}).call(this);
=======
ContentDivManagementBlock.prototype.tagName='fieldset';ContentDivManagementBlock.prototype.className='content-mgnt-block';ContentDivManagementBlock.prototype.container_id='container';ContentDivManagementBlock.prototype.normal_zindex=0;ContentDivManagementBlock.prototype.over_zindex=5;ContentDivManagementBlock.prototype.initialize=function(){var contentCss,slider_block;if($('#content').length!==0){this.container_id='content';}
$("#"+this.container_id).css({'z-index':this.normal_zindex});if((contentCss=localStorage.getItem("#"+this.container_id))){$("#"+this.container_id).css(JSON.parse(contentCss));}
$(this.el).append(this.make('legend',{},'Page content'));slider_block=this.make('div',{"class":'slider-block'});$(this.el).append(slider_block);slider_block.appendChild(this.make('label',{},'Opacity'));slider_block.appendChild(this.contentSlider());$(this.el).append(this.zIndexSwitch());return this.bindEvents();};ContentDivManagementBlock.prototype.zIndexSwitch=function(){var block,label;block=this.make('div',{"class":'zindex-switch'});this.zIndexSwitch=this.make('input',{type:"checkbox"});$(block).append(this.zIndexSwitch);if($("#"+this.container_id).css('z-index')===this.over_zindex){this.zIndexSwitch.checked=true;}
label=this.make('label',{},'Content on top (touch "C")');return $(block).append(label);};ContentDivManagementBlock.prototype.contentSlider=function(){return this.contentSlider=this.make('input',{id:"contentSlider",type:"range",value:$("#"+this.container_id).css('opacity')*100});};ContentDivManagementBlock.prototype.bindEvents=function(){var _this=this;$(this.contentSlider).bind('change',function(){$("#"+_this.container_id).css('opacity',$(_this.contentSlider)[0].value/100);return _this.saveContentCss();});$(this.zIndexSwitch).bind('change',function(event){if(_this.zIndexSwitch.checked){$("#"+_this.container_id).css({'z-index':_this.over_zindex});}else{$("#"+_this.container_id).css({'z-index':_this.normal_zindex});}
return _this.saveContentCss();});return $(window).bind('keypress',function(event){if(event.charCode===99){return $(_this.zIndexSwitch).trigger('click');}});};ContentDivManagementBlock.prototype.render=function(){return this.el;};ContentDivManagementBlock.prototype.saveContentCss=function(){return localStorage.setItem("#"+this.container_id,JSON.stringify({opacity:$("#"+this.container_id).css('opacity'),'z-index':$("#"+this.container_id).css('z-index')}));};return ContentDivManagementBlock;})(Backbone.View);}).call(this);(function(){var __hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child;};OverlayMe.Overlays.ImagesManagementDiv=(function(_super){__extends(ImagesManagementDiv,_super);ImagesManagementDiv.name='ImagesManagementDiv';function ImagesManagementDiv(){return ImagesManagementDiv.__super__.constructor.apply(this,arguments);}
ImagesManagementDiv.prototype.tagName='fieldset';ImagesManagementDiv.prototype.id='images_mgnt';ImagesManagementDiv.prototype.initialize=function(){$(this.el).append(this.make('legend',{},'Overlaying images'));this.controlBlock=this.make('div',{"class":'controls'});$(this.el).append(this.controlBlock);this.controlBlock.appendChild(this.checkAllbox());this.controlBlock.appendChild(this.make('label',{},'All/None'));this.controlBlock.appendChild(this.hideInactiveBox());this.controlBlock.appendChild(this.make('label',{},'Hide Inactives'));this.overlaysListBlock=this.make('div',{"class":'overlays-list'});$(this.el).append(this.overlaysListBlock);return $(this.el).append(this.dynamicAddsBlock());};ImagesManagementDiv.prototype.append=function(block){return this.overlaysListBlock.appendChild(block);};ImagesManagementDiv.prototype.del=function(image_id){$(".overlay-image-block[data-img-id="+image_id+"]",this.el).remove();return $("#images_container #"+image_id).remove();};ImagesManagementDiv.prototype.dynamicAddsBlock=function(){var image_text_input,_this=this;this.dynamicAddsBlock=this.make('div',{"class":'dynamic-adds'});this.dynamicAddsBlock.appendChild(this.make('label',{},'Add an image'));image_text_input=this.make('input',{type:'text',placeholder:"http://"});this.dynamicAddsBlock.appendChild(image_text_input);$(image_text_input).bind('keypress',function(e){if(e.charCode===13){OverlayMe.dyn_manager.addImage(e.target.value);return e.target.value='';}});return this.dynamicAddsBlock;};ImagesManagementDiv.prototype.hideInactiveBox=function(){var _this=this;this.hideInactiveBox=this.make('input',{type:"checkbox","class":'hide-inactive'});if($(this.hideInactiveBox).css('visibility')==='visible'){this.hideInactiveBox.checked=true;}
$(this.hideInactiveBox).bind('change',function(event){_this.hideInactive(_this.hideInactiveBox.checked);return _this.saveState();});return this.hideInactiveBox;};ImagesManagementDiv.prototype.hideInactive=function(state){return _.each($('.overlay-image-block'),function(img_block){var checkbox,img_id;checkbox=$('input[type=checkbox]',img_block);img_id=$(img_block).attr('data-img-id');if(state&&!checkbox.first()[0].checked){$(img_block).hide();return $("#"+img_id).hide();}else{$(img_block).show();return $("#"+img_id).show();}});};ImagesManagementDiv.prototype.checkAllbox=function(){var _this=this;this.checkAllBox=this.make('input',{type:"checkbox","class":'check-all'});if($(this.checkAllBox).css('visibility')==='visible'){this.checkAllBox.checked=true;}
$(this.checkAllBox).bind('change',function(event){_this.checkAll(_this.checkAllBox.checked);return _this.saveState();});return this.checkAllBox;};ImagesManagementDiv.prototype.checkAll=function(state){var checkbox,_i,_len,_ref,_results;_ref=$('.overlay-image-block input[type=checkbox]');_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++){checkbox=_ref[_i];if(checkbox.checked!==state){_results.push($(checkbox).trigger('click'));}else{_results.push(void 0);}}
return _results;};ImagesManagementDiv.prototype.saveState=function(){};ImagesManagementDiv.prototype.render=function(){return this.el;};return ImagesManagementDiv;})(Backbone.View);}).call(this);(function(){var overlay_panel;if(OverlayMe.mustLoad()){overlay_panel=new OverlayMe.MenuItem({id:"overlay_panel",title:"Overlays"});overlay_panel.append(new OverlayMe.Overlays.ContentDivManagementBlock().render());OverlayMe.images_management_div=new OverlayMe.Overlays.ImagesManagementDiv();overlay_panel.append(OverlayMe.images_management_div.render());OverlayMe.loadDefaultImage=function(){if(OverlayMe.dyn_manager.isEmpty()){return OverlayMe.dyn_manager.addImage('https://a248.e.akamai.net/assets.github.com/images/modules/about_page/octocat.png');}};$.ajax({url:'/overlay_images',dataType:'json',success:function(data){if(data.length===0){return OverlayMe.loadDefaultImage();}else{return $.each(data,function(index,img_path){return OverlayMe.images_management_div.append(new OverlayMe.Overlays.Image(img_path).render());});}},error:function(){return OverlayMe.loadDefaultImage();}});$(OverlayMe.Menu).append(overlay_panel.render());$(window).trigger('images_should_be_rendred');$(window).bind('mousemove',function(event){return $(window).trigger('mymousemove',event);});OverlayMe.dyn_manager=new OverlayMe.Overlays.DynamicManager();OverlayMe.dyn_manager.loadAll();window.overlay_me_loaded=true;}}).call(this);(function(){}).call(this);
>>>>>>> 34bbf72... attempt with an erb...:overlay_me/overlay_me.min.js
